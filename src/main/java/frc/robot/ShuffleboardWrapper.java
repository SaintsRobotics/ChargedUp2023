...
public class ShuffleboardWrapper {
   ...
   */
  public static ShuffleboardWrapper getDefault() {
    return m_default;
  }
  ...
  private class GrabberSubsystemExtractor extends GrabberSubsystem {
    public double getPSI() {
      return pHub.getPressure(0);
    }

    public boolean isOpen() {
      return solenoid0.get() == Value.kReverse;
    }
  }

  private class LEDSubsystemExtractor extends LEDSubsystem {
  ...
  private static LEDSubsystemExtractor LED;
  ...
  public ShuffleboardWrapper(String name) {
    m_name = name;
    ShuffleboardLayout matchLay = Shuffleboard.getTab(m_name).getLayout("Match Info", BuiltInLayouts.kList)
  ...
  public void addSubsystem(ArmSubsystem sys) {
    ...
    ShuffleboardLayout armLay = Shuffleboard.getTab(m_name).getLayout("Arm", BuiltInLayouts.kList).withPosition(1, 0)
        .withSize(1, 3);
  ...
  public void addCamera(PhotonCamera cam) {
    ShuffleboardLayout camLay = Shuffleboard.getTab(m_name).getLayout("Camera", BuiltInLayouts.kGrid).withPosition(0, 3)
        .withSize(2, 2);
    camLay.addCamera("Robot", "MS HD", "http://127.0.0.1:1181/stream.mjpg",
        "http://photonvision.local:1181/stream.mjpg", "http://10.18.99:1181/stream.mjpg",
        "http://172.11.22.1:1181/stream.mjpg", "http://172.22.11.2:1181/stream.mjpg");
  }

  public void addSubsystem(DriveSubsystem sys) {
    drive = (DriveSubsystemExtractor) sys;
    ShuffleboardLayout driveLay = Shuffleboard.getTab(m_name).getLayout("Drive", BuiltInLayouts.kGrid)
  ...
  public void addSubsystem(GrabberSubsystem sys) {
    ...
    ShuffleboardLayout grabberLay = Shuffleboard.getTab(m_name).getLayout("Grabber", BuiltInLayouts.kList)
        .withPosition(3, 2).withSize(1, 1);
    grabberLay.addDouble("PSI", () -> grabber.getPSI());
    grabberLay.addBoolean("Open?", () -> grabber.isOpen());
  }

  public void addSubsystem(LEDSubsystem sys) {
    LED = (LEDSubsystemExtractor) sys;
    ShuffleboardLayout LEDLay = Shuffleboard.getTab(m_name).getLayout("LED", BuiltInLayouts.kGrid).withSize(3, 3)
        .withSize(2, 1);
    LEDLay.addString("Color", () -> LED.getColor());
    LEDLay.addBoolean("LED lock", () -> LED.getLock());
  }
}
