...
import frc.robot.commands.AutoAlignCommand.Direction;
...
public class RobotContainer {
  ...
  private void configureButtonBindings() {
    ...
    new POVButton(m_driverController, 0).onTrue(new SnapRotateCommand(m_robotDrive, Math.PI))
        .onTrue(new AutoAlignCommand(m_robotDrive, Direction.kCenter)); // Align to nearest
    new POVButton(m_driverController, 90).onTrue(new SnapRotateCommand(m_robotDrive, Math.PI))
        .onTrue(new AutoAlignCommand(m_robotDrive, Direction.kRight)); // Align to nearest driver right
    new POVButton(m_driverController, 270).onTrue(new SnapRotateCommand(m_robotDrive, Math.PI))
        .onTrue(new AutoAlignCommand(m_robotDrive, Direction.kLeft)); // Align to nearest driver left
  }
        ...
        PathPlanner.loadPathGroup(
            ...
            new PathConstraints(
                AutonConstants.maxVelocity
                    - (path.equals("Charger-comms") ? 0.75
                        : 0),
